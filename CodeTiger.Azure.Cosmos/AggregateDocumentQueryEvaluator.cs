...
internal static partial class AggregateDocumentQueryEvaluator
{
    ...
    /// <see cref="AggregateStoredProcedure"/> object containing all information needed to execute that query in
    /// Cosmos.
    /// </summary>
    /// <param name="expression">An expression representing an aggregate document query.</param>
    /// <returns>An <see cref="AggregateStoredProcedure"/> object containing all information needed to execute the
    /// query in Cosmos.</returns>
    public static AggregateStoredProcedure Evaluate(Expression expression)
    {
        ...
        string storedProcedureBody = new StringBuilder(StoredProcedureTemplate)
            ...
            .Replace(CreateAggregateSeedFunctionPlaceholder, GetCreateAggregateSeedFunction(expressions))
            ...
            .Replace(ShouldStartNewAggregateFunctionPlaceholder, GetShouldStartNewAggregateFunction(expressions))
            .ToString();
    ...
    private static string GetQuery(QueryExpressions expressions, Dictionary<string, object?> queryParameters)
    {
        ...
        if (expressions.GroupByExpression != null)
        {
            ...
            queryBuilder.Append(QueryOrderByClauseVisitor.Visit(expressions.GroupByExpression));
        }
    ...
    private class QueryExpressions
    {
        ...
        {
            if (CreateAggregateExpression != null || GroupByExpression != null || WhereExpressions?.Any() == true)
            {
                throw new NotImplementedException();
            }
