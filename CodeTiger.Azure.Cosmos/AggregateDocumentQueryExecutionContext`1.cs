...
namespace CodeTiger.Azure.Cosmos
{
    ...
    internal class AggregateDocumentQueryExecutionContext<T> : IAggregateDocumentQueryExecutionContext<T>
    {
        ...
        public AggregateDocumentQueryExecutionContext(CosmosContainer container,
        ...
        public async Task<AggregateQueryResponse<T>> ExecuteNextAsync(CancellationToken cancellationToken)
        {
            ...
            var storedProcedure = _container.StoredProcedures[_storedProcedure.Id];

            var response = await storedProcedure
                .ExecuteAsync<AggregateDocumentQueryState, AggregateDocumentQueryState>(
                    _queryOptions.PartitionKey, _queryState, null, cancellationToken)
            ...
            if (response.StatusCode == HttpStatusCode.NotFound)
            {
                ...
                try
                {
                    await _container.StoredProcedures
                        .CreateStoredProceducreAsync(_storedProcedure.Id, _storedProcedure.Body)
                        .ConfigureAwait(false);
                }
                ...
                response = await storedProcedure
                    .ExecuteAsync<AggregateDocumentQueryState, AggregateDocumentQueryState>(
                        _queryOptions.PartitionKey, _queryState, null, cancellationToken)
                    .ConfigureAwait(false);
            }
